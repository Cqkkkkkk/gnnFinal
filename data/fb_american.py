...
class FBDataset(InMemoryDataset):
    ...
    def process(self):
        ...
        local_info_names = {0: "status", 1: "major",
                            2: "second major", 3: "house", 4: "year", 5: "high school"}
        y = metadata[:, 1]
        metadata = np.concatenate([metadata[:, :1], metadata[:, 2:]], axis=1)

        n_types = {0: "person", 1: "status", 2: "major",
                   3: "second major", 4: "house", 5: "year", 6: "high school"}
        items2idxes = []
        ...
        df = pd.DataFrame(
            {local_info_names[idx]: metadata[:, idx] for idx in range(len(local_info_names))})

        for i in range(len(items2idxes)):

            df[local_info_names[i]] = df[local_info_names[i]].map(
                items2idxes[i])
        ...
        for idx, n_type in n_types.items():
            if n_type == "person":
                ...
                indices = torch.arange(
                    len(items2idxes[idx-1]))[None, :].repeat(2, 1)
        ...
        for idx, n_type in n_types.items():
            if n_type != "person":
                ...
                print(temp.shape)
                data[e_type].edge_index = torch.stack(
                    [person_nodes[~valid_index.bool()], torch.tensor(temp, dtype=torch.long)])
                e_type = (n_type, "rev_to", "person")
                data[e_type].edge_index = torch.stack(
                    [torch.tensor(temp, dtype=torch.long), person_nodes[~valid_index.bool()]])

        edge_index = np.stack(graph.nonzero())
        data[("person", "to", "person")].edge_index = torch.tensor(
