...
class Actor(InMemoryDataset):
    ...
    def process(self):
        ...
        nodes_dict = {"idx": defaultdict(list), "name": [], "times": [
        ], "type": [], "words": []}
        edges = []
        ...
        index = np.concatenate([np.arange(num)
                               for num in num_nodes_dict.values()], axis=0)
        nodes2idx = index[np.argsort(nodes, axis=0)]
        print(len(nodes2idx))
        vectorizer = CountVectorizer()

        corpus = list(map(lambda x: " ".join(
            x.split(";")), nodes_dict["words"]))
        ...
        for idx, type in node_types.items():
            ...
            data[type].x = torch.sparse_coo_tensor(
                indices, values, feat.size())
        ...
        train_index, test_index = train_test_split(
            label_idx, test_size=0.2, shuffle=True)
        train_mask, test_mask = index_to_mask(torch.LongTensor(train_index), len(
            label_idx)), index_to_mask(torch.LongTensor(test_index), len(label_idx))
        ...
        for i in range(3):
            for j in range(3):
                edge_idx = np.nonzero((ntypes[edges[:, 0]] == i).astype(
                    int) * (ntypes[edges[:, 1]] == j).astype(int))
                ...
                edge_index, _ = coalesce(
                    edge_index, None, num_nodes_dict[node_types[i]], num_nodes_dict[node_types[j]])
                print(is_undirected(edge_index))
                data[(node_types[i], "rel_{}".format(idx),
                      node_types[j])].edge_index = edge_index
                data[(node_types[j], "rel_{}_rev".format(idx), node_types[i])
...
if __name__ == "__main__":
    ...
    print((data["starring"].y[data[("starring", "rel_0", "starring")].edge_index[0]] == data["starring"].y[data[(
        "starring", "rel_0", "starring")].edge_index[1]]).sum() / data[("starring", "rel_0", "starring")].edge_index.shape[1])
