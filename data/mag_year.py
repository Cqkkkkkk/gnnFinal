...
class MAG_YEAR(InMemoryDataset):
    r"""
    The mag-year dataset is a variant of the obgn-mag datast from the `"Open Graph Benchmark: Datasets for
    Machine Learning on Graphs" <https://arxiv.org/abs/2005.00687>`_ paper. 
    They share the same node features and edges, 
    the core distinction is that the mag-year's label is the year of the paper,
    clustered into five bins for classification.
    The cluster criteria is choosen that The classes ratios are approximately balanced.
    ...
    features.
    - The task is to predict the publication year of the paper. In total, there are 6 different publication year bins.
    ...
    def process(self):
        ...
        # pdb.set_trace()
        data = self.modify_label(data)
        torch.save(self.collate([data]), self.processed_paths[0])

    def modify_label(self, data):
        paper_year = data['paper'].year.numpy()
        bins = [2012, 2014, 2016, 2018, np.inf]
        binned_paper_year = np.digitize(paper_year, bins)
        binned_paper_year = torch.tensor(binned_paper_year)
        # tmp = np.bincount(binned_paper_year)
        data['paper'].y = binned_paper_year
        return data
